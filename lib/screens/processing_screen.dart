...
class _ProcessingScreenState extends State<ProcessingScreen> {
  ...
  final List<_StepItem> _steps = [
    ...
    _StepItem(Icons.hearing, 'Gemini AI ຖອດສຽງລາວ'),
    _StepItem(Icons.subtitles, 'ສ້າງ Subtitle'),
  ];
  int _currentStep = 0;
  ...
  Future<void> _runTranscription(String apiKey) async {
    try {
      ...
      final langCode = switch (project.language) {
        'lo' => 'lo',
        'th' => 'th',
        'en' => 'en',
        _ => 'lo',
      ...
      var segments = await service.transcribe(
        widget.videoPath,
        language: langCode,
        ...
        onProgress: (status) {
          ...
          setState(() {
            ...
            if (status.contains('ດຶງສຽງ')) {
              _currentStep = 0;
              _progress = 0.25;
            } else if (status.contains('ສົ່ງສຽງ') || status.contains('ສົ່ງ')) {
              _currentStep = 0;
              _progress = 0.45;
            } else if (status.contains('ຖອດສຽງ')) {
              _currentStep = 1;
              _progress = 0.75;
      ...
      if (segments.isNotEmpty) {
        ...
        try {
          await LaoWordService.ensureWordUnits(segments, locale: langCode);
        } catch (_) {}

        bool aligned = false;
        // BEST: use a real Whisper timeline (if a Groq key is set). Gemini gives
        ...
        if (groqKey != null && groqKey.isNotEmpty) {
          try {
            ...
            final wt = await GroqSpeechService(
              apiKey: groqKey,
            ).fetchWordTimings(widget.videoPath, language: langCode);
  ...
  Widget _buildErrorView() {
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          isNoKey ? 'ຍັງບໍ່ໄດ້ຕັ້ງ Gemini API Key' : 'ເກີດຂໍ້ຜິດພາດ',
        ...
        if (isNoKey)
          ElevatedButton.icon(
            ...
            label: const Text('ໄປຕັ້ງ Gemini API Key'),
          )
        else
          ElevatedButton.icon(
            onPressed: () {
